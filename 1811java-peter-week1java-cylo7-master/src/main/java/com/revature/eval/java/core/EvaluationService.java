...
public class EvaluationService {
	...
	public String reverse(String string) {
		...
		// Original order of characters in a string
		for (int i = 0; i < len; i++) {
			original[i] = string.charAt(i);
		}

		// Reverse order of characters
		for (int r = 0; r < len; r++) {
			reversed[r] = original[len - 1 - r];
		}
	...
	public String acronym(String phrase) {
		...
		for (int i = 0; i < splitPhrase.length; i++) {
			...
			String firstLetter = Character.toString(splitPhrase[i].charAt(0));
			newAcronym += firstLetter.toUpperCase();
	...
	public String cleanPhoneNumber(String string) {
		...
		String removeChars = "[1+().-]";
		string = string.replaceAll(removeChars, "").replaceAll(" ", "");
		String expectedNumber = string;

		if (expectedNumber.length() != 10) {
			throw new IllegalArgumentException("Must be 10 digits and cannot contain any characters!");				
		} else {				
	...
	public Map<String, Integer> wordCount(String string) {
		String removeChars = "[1+().-]";
		string = string.replace(removeChars, " ");
